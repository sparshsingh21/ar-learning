...
import type { TflLimit } from "@/lib/references";

export function TflDirectory({ entries }: { entries: TflLimit[] }) {
  ...
  const filtered = useMemo(() => {
    ...
    return entries.filter(
      (e) =>
        e.name.toLowerCase().includes(q) || e.tfl.toLowerCase().includes(q),
    );
  }, [entries, query]);
      ...
      <div className="mb-4 flex flex-wrap items-end justify-between gap-3">
        <label className="block min-w-[240px] flex-1">
          ...
          <input
            ...
            placeholder="e.g. Humana, Medicaid TX, BCBS…"
        ...
        <table className="w-full min-w-[520px] text-left text-sm">
          <thead className="bg-accent-soft text-fg">
            <tr>
              ...
              <th className="px-3 py-2.5 font-semibold">TFL</th>
            </tr>
          </thead>
          <tbody>
            {filtered.map((row) => (
              <tr
                key={`${row.name}-${row.tfl}`}
                className="border-t border-border/80 odd:bg-bg-elevated"
              >
                <td className="px-3 py-2 font-medium text-fg">{row.name}</td>
                <td className="px-3 py-2 text-fg-muted">{row.tfl}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
